...
{
    $stmt = $GLOBALS['con']->prepare("SELECT username,password FROM users WHERE username = ? AND password = ? LIMIT 1") OR die ($GLOBALS['con']->error);
...
{
    $s = $GLOBALS['con']->prepare("SELECT description, quantity FROM product ORDER BY quantity ASC LIMIT 0,10");
    $s->execute();
    $result = $s->get_result();
   $data = array();
